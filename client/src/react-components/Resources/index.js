...
export class Resources extends React.Component {
    ...
    plot_resource_icon(resource) {
        ...
        key = {uid(resource)} >
            < Image
        size = "medium"
        src = {"/file_icons/" +resource.type + ".svg"}
        />
        < Card.Content >
        < Card.Header > {resource.name} < /Card.Header>
        < Card.Meta >
        < span
        className = 'date' > {resource.date} < /span>
            < /Card.Meta>
            < Card.Description >
    ...
    render() {
        ...
        < Menu.Item
        header
        className = {"resources_header_text"} > {`${courseName} Resources`
    }<
        /Menu.Item>
        < Menu.Item
